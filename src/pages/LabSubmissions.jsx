    ...
    return (
        <div className="max-w-5xl mx-auto p-6">
            <button onClick={() => navigate(-1)} className="flex items-center text-secondary-foreground hover:text-foreground mb-6">
                <ArrowLeft className="w-4 h-4 mr-2" /> Back to Lab Task
            </button>

            <h1 className="text-2xl font-bold mb-2">Lab Submissions</h1>
            <p className="text-secondary-foreground mb-6">Total: {submissions.length}</p>
                ...
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-border">
                        {submissions.length === 0 ? (
                            <tr><td colSpan="5" className="p-8 text-center text-secondary-foreground">No submissions finding.</td></tr>
                        ) : (
                            submissions.map(sub => (
                                <tr key={sub._id} className="hover:bg-background/50 transition-colors">
                                    ...
                                    <td className="p-4">
                                        {sub.status === 'graded' ? (
                                            <span className="inline-flex items-center gap-1 text-xs font-medium bg-green-100 text-green-700 px-2 py-1 rounded-full"><CheckCircle size={12} /> Graded</span>
                                        ) : (
                                            <span className="inline-flex items-center gap-1 text-xs font-medium bg-yellow-100 text-yellow-700 px-2 py-1 rounded-full"><Clock size={12} /> Pending</span>
                                        )}
                                    </td>
                                    <td className="p-4 text-sm text-secondary-foreground">{new Date(sub.submittedAt).toLocaleString()}</td>
